...
//----------------- convert button start -----------------------------------
    cexconv_button.connect_clicked(glib::clone!(@weak cdirectory1_combobox, @weak cdirectory_o_combobox, @weak ctarget_entry, @weak cnumrows_entry, @weak cutc_entry, @weak progress_progressbar, @weak messageval_label  => move|_| {
...
// linux command iconv -f format -t UTF-8 filename -o outputfile    will convert file to UTF-8   
        ...
        if bolok {
            let strutc = cutc_entry.text();
            numutc = strutc.parse().unwrap_or(-99);
            if numutc < -23 || numutc > 23 {
                messageval_label.set_markup("<span color=\"#FF000000\">********* INVALID NUMBER IN UTC OFFSET ENTRY **********</span>");
                bolok = false;
            }
        }
        if bolok {         
...
//                                  writeln!(&mut targetfile,"line num:{} size value: -{}-", linenum, sizval).unwrap();
                                  ...
                                  if numutc != 0 {
                                              ...
                                              if errcode == 0 {
                                                  for indk in 0..lentime1 {
                                                       ...
                                                       if time_int == -9999 {
                                                          ...
                                                          match indk {
                                                           0 => datehr1 = time_int as i64,
                                                           1 => datemin1 = time_int as i64,
                                                           2 => datesec1 = time_int as i64,
                                                           _ => errcode = 6,
                                                          }
                                                       }
                                                   }
                                      ...
                                      if errcode == 0 {
//                                          println!("yr:-{}- mo:-{}- day: {} hr: {} min: {} sec: {} date {} time {}", dateyr1, datemo1, dateday1, datehr1, datemin1, datesec1, sdate, stime);
                                          let mut dateto = Utc.ymd(dateyr1 as i32, datemo1 as u32, dateday1 as u32).and_hms_milli(datehr1 as u32, datemin1 as u32, datesec1 as u32, 0);
