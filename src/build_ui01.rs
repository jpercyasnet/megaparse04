...
//----------------- source file  button start -----------------------------------
    cdirectory1_button.connect_clicked(glib::clone!(@weak window, @weak cdirectory1_combobox, @weak messageval_label => move|_| {
        ...
        let dialog = FileChooserDialog::new(Some("Choose a XML file"), Some(&window.clone()), FileChooserAction::Open);
        dialog.add_button("Cancel", gtk::ResponseType::Cancel);
        dialog.add_button("Open", gtk::ResponseType::Accept);
        let result = dialog.run();
        if result == gtk::ResponseType::Accept {
            if let Some(uri) = dialog.filename() {
                let filename = uri.into_os_string().into_string().unwrap();
                cdirectory1_combobox.prepend_text(&filename);
...
//----------------- target directory button start -----------------------------------
    cdirectory_o_button.connect_clicked(glib::clone!(@weak window, @weak cdirectory_o_combobox, @weak messageval_label => move|_| {
        ...
        let dialog = FileChooserDialog::new(Some("Choose output Directory"), Some(&window.clone()), FileChooserAction::SelectFolder);
        dialog.add_button("Cancel", gtk::ResponseType::Cancel);
        dialog.add_button("Open", gtk::ResponseType::Accept);
        let result = dialog.run();
        if result == gtk::ResponseType::Accept {
            if let Some(uri) = dialog.filename() {
                let foldername = uri.into_os_string().into_string().unwrap();
